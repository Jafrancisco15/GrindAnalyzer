...
export default function App() {
  ...
  function analyze() {
    try {
      ...
      // Precisión RBP + spreads
      const rbp  = computePrecisionRangeBased(filtered, { d10, d50, d90 });
      const spr  = computeSpreadStats(filtered, { d50 });

      setResults({
        N, d10, d50, d90,
        span: d10>0 ? d90/d10 : 0,
        gsd: spr.gsd, cv: spr.cv, iqrMd: spr.iqrMd,
        precision: rbp.score,
        bandLo: rbp.band[0], bandHi: rbp.band[1],
        shareMain: rbp.shareMain,
        finesFrac: rbp.finesFrac,
        boulderFrac: rbp.boulderFrac
      });

      setViz("mask");
      setStatus(N
        ? `Listo. N=${N} | D50=${d50.toFixed(1)} µm | Precisión=${rbp.score}/100`
  ...
  // --- NUEVO: Precisión RBP (centrado en D50, ventana ±15% o ±40 µm) ---
  function computePrecisionRangeBased(d_um, qs) {
    const arr = (d_um || []).filter(v => isFinite(v) && v > 0).sort((a,b)=>a-b);
    const N = arr.length;
    if (N < 5) return { score: 0, band:[0,0], shareMain:0, finesFrac:0, boulderFrac:0 };

    const d50 = qs?.d50 ?? quantile(arr,0.50);
    const d90 = qs?.d90 ?? quantile(arr,0.90);

    const rel = 0.15;     // ±15%
    const absMin = 40;    // µm
    const win = Math.max(absMin, rel * d50);
    const lo = d50 - win, hi = d50 + win;

    // pesos ~ área 2D (d^2). Si prefieres volumen, usa d^3.
    const weight = (v)=> v*v;
    const totalW = arr.reduce((s,v)=>s + weight(v), 0) || 1;
    const mainW  = arr.reduce((s,v)=>s + ((v>=lo && v<=hi) ? weight(v) : 0), 0);
    const finesW = arr.reduce((s,v)=>s + ((v<100) ? weight(v) : 0), 0);  // <100 µm
    const bloLo  = Math.max(hi, d90);
    const boulW  = arr.reduce((s,v)=>s + ((v>bloLo) ? weight(v) : 0), 0);

    const shareMain   = mainW  / totalW;
    const finesFrac   = finesW / totalW;
    const boulderFrac = boulW  / totalW;

    const score = Math.round(100 * (0.60*shareMain + 0.25*(1-finesFrac) + 0.15*(1-boulderFrac)));
    return { score, band:[lo,hi], shareMain, finesFrac, boulderFrac };
  }

  // --- Spreads clásicos para mostrar (GSD/CV/IQR) ---
  function computeSpreadStats(data, qs) {
    const a = (data || []).filter(v=>v>0).sort((x,y)=>x-y);
    if (!a.length) return { gsd:0, cv:0, iqrMd:0 };
    const d50 = qs?.d50 ?? quantile(a,0.5);
    const q1  = quantile(a,0.25), q3 = quantile(a,0.75);
    const mean = a.reduce((s,v)=>s+v,0)/a.length;
    const sd = stdDev(a);
    const logs = a.map(v=>Math.log(v));
    const mlog = logs.reduce((s,v)=>s+v,0)/logs.length;
    const sdLog = Math.sqrt(logs.reduce((s,v)=>s+(v-mlog)*(v-mlog),0)/logs.length);
    const gsd = Math.exp(sdLog);
    return { gsd, cv: mean>0? sd/mean : 0, iqrMd: d50>0 ? (q3-q1)/d50 : 0 };
  }

  // ---------- Histograma (con D10/D50/D90 + ventana sombreada) ----------
  useEffect(() => {
    ...
    // ventana sombreada (±15% o ±40µm)
    const bandLo = results.bandLo, bandHi = results.bandHi;
    function xFor(v){ return pad + ((v - min) / (max - min || 1)) * plotW; }
    if (bandHi > bandLo && isFinite(bandHi) && isFinite(bandLo)) {
      const xL = xFor(bandLo), xH = xFor(bandHi);
      g.save();
      g.fillStyle = "rgba(253, 224, 71, 0.20)"; // amarillo suave
      g.fillRect(Math.min(xL,xH), pad, Math.abs(xH-xL), plotH);
      g.restore();
      g.fillStyle = "#374151"; g.font = "12px sans-serif";
      g.fillText(`% en ventana: ${Math.round((results.shareMain||0)*100)}%`, pad, pad - 12);
    }
    ...
    const drawVLine = (x, color, label) => {
      ...
      const val = label==='D50'?d50:(label==='D10'?d10:d90);
      const txt = `${label} ${isFinite(val)?fmt(val,0):'—'}µm`;
  ...
  }, [sizes, results.d10, results.d50, results.d90, results.bandLo, results.bandHi, results.shareMain]);

  // ---------- Exportar PDF ----------
  function exportPDF() {
    if (!results.N) { setStatus("No hay resultados para exportar. Ejecuta Analizar primero."); return; }
    const JSPDF = getJsPDF();
    if (!JSPDF) { setStatus("jsPDF no disponible. Asegura el script CDN en index.html."); return; }
    ...
    const rows = [
      ...
      [`% en ventana (±15%)`, `${Math.round((results.shareMain||0)*100)}%`],
      [`Fines <100µm (peso)`, `${Math.round((results.finesFrac||0)*100)}%`],
      [`Boulders (cola alta)`, `${Math.round((results.boulderFrac||0)*100)}%`],
      [`Índice de precisión (0–100)`, String(Math.round(results.precision))],
      [`Ventana`, results.bandHi>results.bandLo ? `${fmt(results.bandLo,0)}–${fmt(results.bandHi,0)} µm` : "—"],
      [`Diámetro canasto (mm)`, String(mm)],
      [`Escala`, scaleStr],
      [`ROI`, roiStr]
    ];
    const col1 = margin, col2 = 92;
    rows.forEach(r => {
    ...
    if (hc) {
      ...
      if (y + h > 297 - margin) { doc.addPage(); y = margin; }
      doc.text("Histograma con D10/D50/D90 + ventana", margin, y); y += 4;
      doc.addImage(imgData, "PNG", margin, y, w, h, undefined, "FAST");
      y += h + 6;
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-3">
        ...
        <div className="border rounded p-3">
          ...
          <ul className="text-sm space-y-1 mt-2">
            ...
            <li><span className="text-gray-600">% en ventana (±15%):</span> {results.N ? Math.round((results.shareMain||0)*100) : "—"}%</li>
            <li><span className="text-gray-600">Fines &lt;100 µm (peso):</span> {results.N ? Math.round((results.finesFrac||0)*100) : "—"}%</li>
            <li><span className="text-gray-600">Boulders (cola alta):</span> {results.N ? Math.round((results.boulderFrac||0)*100) : "—"}%</li>
          ...
          <p className="text-xs text-gray-500 mt-2">
            Banda amarilla = ventana alrededor de D50. Líneas (D10 naranja, D50 negro, D90 verde).
          </p>
        </div>
      </div>
    </div>
